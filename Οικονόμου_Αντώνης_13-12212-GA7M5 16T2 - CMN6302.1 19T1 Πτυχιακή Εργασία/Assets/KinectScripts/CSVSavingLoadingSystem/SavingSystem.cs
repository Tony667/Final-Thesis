...
{   
    ...
    public string mainFileToSave; // main file names --> ExpertMovement, AmatureMovement
    public string secondaryFileToSave; // secndary file names --> Movemets, Names of the players etc 
    public string movementName; // movement names --> movement_01
    ...
    private CalculateJointScore calculateJointScore;
    ...
    private void Awake()
    {
        ...
        calculateJointScore = GetComponent<CalculateJointScore>();
        ...
        if (manager && manager.IsInitialized())
		{
			if(manager.IsUserDetected())
			{
				...
				if(isSaving)
				{
                    ...
                    // save values continuously or for a set amount of time
                    ...
                    // stop the saving process and call the appropriate function to calculate the score
                    else if ((Time.time - saveStartTime) >= secondsToSave)
                    {
                        calculateJointScore.compare = true;
                        isSaving = false;
                    }
                }
			}
